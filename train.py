# train.py—optimized

import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader
import pickle

from model.transformer import MiniLLM
from utils import build_vocab

# 1) Prepare data (on CPU)
with open("data/tiny.txt", encoding="utf-8", errors="ignore") as f:
    raw = f.read(100_000)               # start small for speed
char2idx, idx2char = build_vocab(raw)
data = torch.tensor([char2idx[c] for c in raw])

seq_len = 32
class CharDataset(Dataset):
    def __init__(self, data, seq_len):
        self.data = data
        self.seq_len = seq_len
    def __len__(self):
        return len(self.data) - self.seq_len
    def __getitem__(self, i):
        x = self.data[i : i+self.seq_len]
        y = self.data[i+1 : i+1+self.seq_len]
        return x, y

dataset = CharDataset(data, seq_len)
loader  = DataLoader(dataset, batch_size=128, shuffle=True, num_workers=2)

# 2) Model setup (move to GPU if available)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model      = MiniLLM(vocab_size=len(char2idx), embed_dim=64).to(device)
loss_fn    = nn.CrossEntropyLoss()
optimizer  = optim.Adam(model.parameters(), lr=3e-3)

# 3) Training loop over batches
epochs = 20    # start small—watch your loss
for epoch in range(epochs):
    total_loss = 0.0
    for x_batch, y_batch in loader:
        x_batch = x_batch.to(device)
        y_batch = y_batch.to(device)

        optimizer.zero_grad()
        out, _ = model(x_batch)
        loss   = loss_fn(out.view(-1, out.size(-1)), y_batch.view(-1))
        loss.backward()
        optimizer.step()

        total_loss += loss.item()

    print(f"Epoch {epoch+1}/{epochs}, Avg Loss: {total_loss/len(loader):.4f}")

# 4) Save final model + vocab in one shot
torch.save({
    'model_state': model.state_dict(),
    'char2idx':    char2idx,
    'idx2char':    idx2char
}, "mini_model.pth")

print("✅ Training complete and model saved.")